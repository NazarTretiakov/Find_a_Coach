<template>
  <div class="recommendations">
    ...
    <ul class="recommendations-items">
      <li v-for="recommendation in visibleRecommendations" :key="recommendation.recommendationId" class="recommendations-items_recommendation" :class="isReceivedRecommendationsVisible ? 'recommendations-items_recommendation-received' : 'recommendations-items_recommendation-given'">
        ...
        <div v-if="isGivenRecommendationsVisible" class="recommendations-items_recommendation-delete" @click="deleteRecommendation(recommendation.recommendationId)">
          <span class="recommendations-items_recommendation-delete-inscription">Delete recommendation</span>
        </div>
      </li>
...
export default defineComponent({
  setup() {
    ...
    const loadRecommendations = async () => {
      const result = await useGetAllRecommendations(authStore.userId)
      if (typeof result === 'object' && 'isSuccessful' in result && !result.isSuccessful) {
        router.push('/error-page')
        return
      }
      const recommendations = result as Recommendation[]
      receivedRecommendations.value = recommendations.filter(r => r.recommendedUserId === authStore.userId)
      givenRecommendations.value = recommendations.filter(r => r.recommenderUserId === authStore.userId)
    }
