<template>
  ...
  <div v-else class="survey">
    ...
    <ul class="survey-comments">
      ...
      <li v-if="!isLoading && survey.comments.length == 0" class="survey-comments_empty-state">
        <img class="survey-comments_empty-state-icon" src="@/assets/images/icons/empty-state-icon.svg" alt="Empty state icon">
        <span class="survey-comments_empty-state-inscription">No comments have been added. You can be first.</span>
      </li>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    return { survey, isLoading, toggleLike, toggleSave , vote, answers, isUserAlreadyVoted, getPercentage, inputFieldAddCommentContent, activeUserEmail, createComment, formatDate, deleteComment, loadMoreComments, isLoadMoreCommentsButtonVisible }
  },
})
</script>
