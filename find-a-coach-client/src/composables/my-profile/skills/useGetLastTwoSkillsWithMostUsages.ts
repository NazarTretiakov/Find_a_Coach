...
export default async function useGetTwoSkillsWithMostUsages(userId: string): Promise<Result | Skill[]> {
  try {
    const token = await useEnsureValidToken()

    const response = await fetch(`${API_URL}/get-two-skills-with-most-usages?userId=${userId}`, {
      method: 'GET',
      headers: {
