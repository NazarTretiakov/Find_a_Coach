...
export default async function useAddRecommendation(formData: RecommendationForm): Promise<Result> {
  try {
    ...
    if (!response.ok) {
      ...
      return {
        isSuccessful: false,
        errorMessage: responseData.errorMessage || 'Unexpected error occurred while adding recommendation.'
      }
    }
