...
export default async function useGetAllPositions(userId: string): Promise<Result | Position[]> {
  try {
    const token = await useEnsureValidToken()

    const response = await fetch(`${API_URL}/get-all-positions?userId=${userId}`, {
      method: 'GET',
      headers: {
