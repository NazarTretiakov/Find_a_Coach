...
export default async function useGetSavedActivities(userId: string, page: number, pageSize: number): Promise<Result | ActivitiesPaged> {
  try {
    ...
    const response = await fetch(`${API_URL}/get-saved-activities?userId=${userId}&page=${page}&pageSize=${pageSize}`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      ...
      return {
        isSuccessful: false,
        errorMessage: responseData.errorMessage || 'Unexpected error occurred while getting saved activities.',
      }
    }
