...
export default async function useAddLanguage(formData: LanguageForm): Promise<Result> {
  try {
    ...
    if (!response.ok) {
      ...
      return {
        isSuccessful: false,
        errorMessage: responseData.errorMessage || 'Unexpected error occurred while adding language.'
      }
    }
